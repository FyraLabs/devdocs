...
Anda supports automatically updating packages via the `anda update` command. The command finds all the projects with the file `update.rhai` in the same directory or if the configuration file has the `update` property set to a value:
