...
updates (`anda update`) or just scripting in general.
...
`.rhai`, anda will run these files using `sh -c path/to/file`.

For more information about `project.*.update` (`update.rhai`), check out the
...
print(sh("ls -alh", #{ "cwd": "anda/tools/umstellar/" })); // prints out the command output in real time, then: `#{ "outcome": "success", "ctx": … }`
...
`anda update` execute all update scripts found in the repository recursively as
defined by the `project.*.update` entry in `anda.hcl` separately:
