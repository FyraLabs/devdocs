...
Consider the following. For `{ }` and `""`, they represent, respectively, a dictionary (empty by default) with keys and string values, and a string with no default value. Unless otherwise stated, all the attributes are optional.
