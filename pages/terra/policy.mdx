...
(b) and (c) MUST be used with `labels { nightly = 1 }{:hcl}` in `anda.hcl`, since they rely on the
assumption that there is 1 `%shortcommit` for each `%commit_date` and vice versa.
