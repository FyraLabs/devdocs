...
Packages without their _tags_ specified in `Name:{:rpmspec}` MUST be regarded as stable.
...
We encourage the use of [`mold`](https://github.com/rui314/mold), which may speed up build times especially in large projects. You may enable it in C/++ projects by adding `-fuse-ld=mold` in `CFLAGS`/`CXXFLAGS`.
...
Alternatively, use the `_online` macro variants:
```
%prep
%goprep
%autopatch -p1
