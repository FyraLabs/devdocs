...
All graphical applications that can be started by the user MUST provide a [desktop entry](https://wiki.archlinux.org/title/Desktop_entries#Application_entry) file.
Additionally, all packages except libraries MUST provide an [appstream metadata](https://www.freedesktop.org/software/appstream/docs/chap-Quickstart.html#qsr-app-example) file.
...
- It is RECOMMENDED to use `%cargo_license_online` and `%cargo_license_summary_online`.
...
For `nimble`, use the `%nim_prep` and `%nim_c src/pkgname` macros. If `atlas` is used, run it directly.
...
Shell completion files warrant their own subpackages. You SHOULD use `%pkg_completion` to automatically
