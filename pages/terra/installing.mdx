...
<Callout type="info">
    Subrepos are only available for `f*` branches (not `el*`).
</Callout>
...
The extras subrepo provides packages that conflict with Fedora but do not belong to the rest of the subrepos.

### Mesa

In the Mesa subrepo, the `mesa` package is provided. Unlike Fedora and RPM Fusion, this build of mesa is complete, coherent, and with most if not all features enabled. This also contains patches from Universal Blue.

### NVIDIA

The NVIDIA subrepo provides a different set of NVIDIA drivers based on the [Negativo17](https://negativo17.org/) builds.

### Multimedia

The multimedia subrepo is only available to Fedora 43 and newer releases. Multimedia-related packages, including `gstreamer1` packages, `ffmpeg`, and other codecs are shipped in this subrepo.
