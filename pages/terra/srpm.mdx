...
We provide custom SRPM macros to ease packaging and better integrate into our workflow. Please use these macros when possible when contributing to Terra.
...
If the file is an `.svg`, use the [`%_scalableiconsdir{:rpmspec}`](#_scalableiconsdir).
...
If the file is a `.png`, use the [`%_hicolordir{:rpmspec}` macro](#_hicolordir) detailed above.
...
#### %_nvm_dir
The location in the `%{rpmbuilddir}{:rpmspec}` what will be passed to `$NVM_DIR{:bash}`.
...
This is similar to [`%__npm_license_checker{:rpmspec}`](#__npm_license_checker), but limits the output to only the license.
...
1. The JavaScript package manager cache directories are placed in [`%{rpmbuilddir}{:rpmspec}`](#rpmbuilddir) because `$HOME{:bash}` can be different depending on if a build is in Mock or RPM Build.
...
However the proposed environment variable is `$BUN_HOME{:bash}`, so we have this for the future.
...
**NOTE:** `This macro does NOT install the JavaScript package managers used for builds (e.g., `nodejs-npm`, `bun`, `pnpm`, `yarnpkg`, `yarnpkg-berry`). These must still be specified in the `BuildRequires:{:rpmspec}` tag manually.
...
>>>>>>> e71f8f7ed041580a9626c344506d417c9d35cf64
