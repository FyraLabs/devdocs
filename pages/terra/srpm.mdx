...
### %_gnomeextensionsdir
Expands to: `%{_datadir}/gnome-shell/extensions/%{uuid}{:rpmspec}` or `%{_datadir}/gnome-shell/extensions{:rpmspec}`

**NOTE:** If the `%{uuid}{:rpmspec}` macro is defined, it will be appended to the path upon expansion: `%{_datadir}/gnome-shell/extensions/%{uuid}{:rpmspec}`.
Otherwise, it will expand without a `uuid`, which MUST be added manually: `%{_datadir}/gnome-shell/extensions{:rpmspec}`.
