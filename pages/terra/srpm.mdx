...
<Callout>
  `bun audit` is a currently just a wrapper for NPM audit but lacks the `fix` option; use `%npm_audit_fix{:rpmspec}` to fix vulnerabilities.
</Callout>
...
<div style={{ float: "right", margin: "1em" }}>
  | `%{_target_cpu}` | `%{_electron_cpu}{:rpmspec}` |
  |------------------|------------------------------|
  | `x86_64`         | `x64`                        |
  | `aarch64`        | `arm64`                      |
  | `x86`            | `ia32`                       |
