...
If you would like to add/fix/update these macros, or MUST raise an issue, please do so in the [SRPM macros repository](https://github.com/terrapkg/srpm-macros), or reach out to us somewhere [here](https://wiki.ultramarine-linux.org/en/community/community/).
...
Used in the [`%pkg_completions{:rpmspec}`](#pkg_completion) macro, or manually when `%pkg_completions{:rpmspec}` cannot be used.
...
If the file is a `.svg`, use the [`%_scalableiconsdir{:rpmspec}`](#_scalableiconsdir).
...
If the file is a `.png`, use the [`%_hicolordir{:rpmspec}`](#_hicolordir) detailed above.
...
While not macros proper, some Terra, Fedora, and even builtin RPM macros define environment variables which may be of use in specs. These can be as simple as defining a string of text, to being commands or directories.
...
### Node.js Macros
Macros for using [Node.js](https://nodejs.org). These require use of the [Fedora Node.js macros](https://docs.fedoraproject.org/en-US/packaging-guidelines/Node.js/#_macros) in the `nodejs-packaging` package.
...
#### %vendor_nodejs
this maco SHOULD only be used if Node.js packages available from the repos are not usable for some reason.
...
#### %fetch_node_tests
Fetch self-tests from a Git repo. MUST include all necessary test files and folders. If used, SHOULD be used in `%prep{:rpmspec}`.
...
You MUST use [`%fetch_node_tests{:rpmspec}`](#fetch_node_tests) first if using this.
...
1. The JavaScript package manager cache directories are placed in [`%{rpmbuilddir}{:rpmspec}`](#rpmbuilddir) because `$HOME` can be different depending on if a build is in Mock or RPM Build.
...
NOTE: Bun audit is a currently a wrapper for NPM audit but lacks the `fix` option, in order to fix vulnerabilities you need to use `%npm_audit_fix{:rpmspec}`.
...
Expands to: `/usr/bin/env %{pnpm_common_envvars} %{?with_vendored_pnpm:%{rpmbuilddir}/pnpm/pnpm}%{!?with_vendored_pnpm:/usr/bin/pnpm}{:rpmspec}`
...
#### %pnpm_audit
Audit with PNPM if package uses bundled Node dependencies. For use in the `%check{:rpmspec}` section.
...
#### %yarn_audit
Audit Yarn if package uses bundled Node dependencies. For use in the `%check{:rpmspec}` section.
...
This ensures the environment variables are explicitly exported and are more likely to be picked up by all shell processes. It also creates an environment variable called `$NPMFLAGS` which can be passed to NPM in lieu of the macro `%{npm_buildflags}{:rpmspec}` to force our config, if necessary.
...
Does NOT install the JavaScript package managers used for builds (e.g., `nodejs-npm`, `bun`, `pnpm`, `yarnpkg`, `yarnpkg-berry`). These must still be specified in the `BuildRequires:{:rpmspec}` tag manually.
...
- Converts `%{_target_cpu}{:rpmspec}` (the RPM build architecture) into `%{_electron_cpu}{:rpmspec}`, the Electron format for architectures it is compatible with.
  - Defines `%{_electron_cpu}{:rpmspec}` for x86_64 architectures as `x64`.
  - Defines `%{_electron_cpu}{:rpmspec}` for aarch64 architectures as `arm64`.
  - Defines `%{_electron_cpu}{:rpmspec}` for x86 architectures as `ia32`.
  - Defines `%{_electron_cpu}{:rpmspec}` for armv7l architectures as `armv7l`.
  - Defines `%{_electron_cpu}{:rpmspec}` for mips64 architectures as `mips64el`.
    - These are defined because Electron names the build files and directories after the architecture.
    - The macros used to build Electron apps (`%npm_build{:rpmspec}`, `%bun_build{:rpmspec}`, `%pnpm_build{:rpmspec}`, and `%yarn_build{:rpmspec}`) all require this to be set.
...
- Sets `%debug_package{:rpmspec}` to `%{nil}{:rpmspec}` to prevent stripping of bundled libraries and/or build failures due to not enough data to strip.
...
- SHOULD come before the `License:{:rpmspec}` field if using [`%{electron_license}{:rpmspec}`](#electron_license).
...
- `-r{:rpmspec}`: Executes `%{__bun} run{:rpmspec}` for scripts in the `package.json`. Expects an argument.
- `-v{:rpmspec}`: Run Electron Vite build steps. Not needed for all Electron apps.
- `-F{:rpmspec}`: Runs `--force{:rpmspec}` on `%{__bun} install{:rpmspec}` step. It will make sure all dependencies are the latest versions from the registry, reinstalling them if necessary.
- `-N{:rpmspec}`: Runs `--no-save{:rpmspec}` on `%{__bun} install{:rpmspec}` step. Prevent updating or (re)creating a lockfile to prevent freezing.
...
## sccache
These macros are fur using [sccache](https://github.com/mozilla/sccache).
...
This is only for use in the [`%pkg_devel_files`](#pkg_devel_files) macro.
...
A git URL and a `%version{:rpmspec}` or `%commit{:rpmspec}` SHOULD be passed to this macro.
...
Used as a macro within [`%pkg_devel_files{:rpmspec}`](#pkg_devel_files).

### %__anda_libsfiles
Expands to: `%_anda_srpm_macros_dir/find-libsinfo.sh %buildroot > anda-libsfiles.list{:rpmspec}`
Used as a macro within [`%pkg_libs_files{:rpmspec}`](#pkg_libs_files).

### %_anda_srpm_macros_dir
Expands to: `/usr/libexec/anda-srpm-macros`
Anda directory for scripts such as `find-develinfo.sh` used by `%__anda_develfiles{:rpmspec}`, which is jused by [`%pkg_devel_files{:rpmspec}`](#pkg_devel_files).

### %__anda_staticfiles
Expands to: `%_anda_srpm_macros_dir/find-staticinfo.sh %buildroot > anda-staticfiles.list{:rpmspec}`
Creates anda-staticfiles.list file in `%{buildroot}{:rpmspec}` to be used by [`%pkg_static_files{:rpmspec}`](#pkg_static_files).

### %is_electron_bundled
Expands to `%bcond bundled_electron %["%{name}" != "electron"]{:rpmspec}`
A functional macro that tells other macros if Electron is bundled or if the package being worked with is Electron itself. Used by [`%electronmeta{:rpmspec}`](#electronmeta) and [`%electron_license{:rpmspec}`](#electron_license).
