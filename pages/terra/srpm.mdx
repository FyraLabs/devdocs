...
Used in the [`%pkg_completions{:rpmspec}` macro](#pkg_completion), or manually when `%pkg_completions{:rpmspec}` cannot be used.
...
If the file is an `.svg`, use the [`%_scalableiconsdir{:rpmspec}` macro](#_scalableiconsdir).
...
You MUST use the [`%fetch_node_tests{:rpmspec}` macro](#fetch_node_tests) first if using this.
...
Used as a macro within the [`%pkg_devel_files{:rpmspec}` macro](#pkg_devel_files).

### %__anda_libsfiles
Expands to: `%_anda_srpm_macros_dir/find-libsinfo.sh %buildroot > anda-libsfiles.list{:rpmspec}`
Used as a macro within the [`%pkg_libs_files{:rpmspec}` macro](#pkg_libs_files).

### %_anda_srpm_macros_dir
Expands to: `/usr/libexec/anda-srpm-macros`
Anda directory for scripts such as `find-develinfo.sh` used by `%__anda_develfiles{:rpmspec}`, which is used by the [`%pkg_devel_files{:rpmspec}` macro](#pkg_devel_files).
