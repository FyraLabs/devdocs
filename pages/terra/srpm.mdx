...
Used in the [`%pkg_completions{:rpmspec}` macro](#pkg_completion) macro, or manually when `%pkg_completions{:rpmspec}` cannot be used.
...
You MUST use [`%fetch_node_tests{:rpmspec}` macro](#fetch_node_tests) first if using this.
...
<div style={{ float: "right", margin: "1em 0em 1em 0.5em" }}>
  | `%_target_cpu` | `%_electron_cpu` |
  |:--------------:|:----------------:|
  | [%{x86_64}](https://github.com/rpm-software-management/rpm/blob/4a9b7b5908d8b463a836b51322242677677bd8b7/macros.in#L1126)    | `x64`            |
  | [%{arm64}](https://github.com/rpm-software-management/rpm/blob/4a9b7b5908d8b463a836b51322242677677bd8b7/macros.in#L1138)     | `arm64`          |
  | [%{ix86}](https://github.com/rpm-software-management/rpm/blob/4a9b7b5908d8b463a836b51322242677677bd8b7/macros.in#L1122)      | `ia32`           |
  |`armv7l`, `armv7hl`, `armv7hnl`| `armv7l`         |
  | [%{mips64}](https://github.com/rpm-software-management/rpm/blob/4a9b7b5908d8b463a836b51322242677677bd8b7/macros.in#L1146)    | `mips64el`       |
</div>

Rundown of what all `%electronmeta{:rpmspec}` does:
- Converts `%_target_cpu{:rpmspec}` (the RPM build architecture) into `%_electron_cpu{:rpmspec}`, the Electron format for compatible architectures. See the table to the right for conversions.
...
- SHOULD come before the `License:{:rpmspec}` field if using the [`%{electron_license}{:rpmspec}` macro](#electron_license).
...
Anda directory for scripts such as `find-develinfo.sh` used by `%__anda_develfiles{:rpmspec}`, which is used in the [`%pkg_devel_files{:rpmspec}` macro](#pkg_devel_files).

### %__anda_staticfiles
Expands to: `%_anda_srpm_macros_dir/find-staticinfo.sh %buildroot > anda-staticfiles.list{:rpmspec}`
Creates anda-staticfiles.list file in `%{buildroot}{:rpmspec}` to be used in the [`%pkg_static_files{:rpmspec}` macro](#pkg_static_files).
