...
Used in the [`%pkg_completions`](#pkg_completion) macro, or manually when `%pkg_completions{:rpmspec}` cannot be used.
...
Anda directory for scripts such as `find-develinfo.sh` used by `%__anda_develfiles{:rpmspec}`, which is jused by [`%pkg_devel_files`](#pkg_devel_files).
