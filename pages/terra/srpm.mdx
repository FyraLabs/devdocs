...
- Converts `%_target_cpu{:rpmspec}` (the RPM build architecture) into `%_electron_cpu{:rpmspec}`, the Electron format for compatible architectures. See the table below for conversions.
...
<div style={{ margin: "1em 0em 1em 0.5em" }}>
  |     `%_target_cpu` Macro       |     Corresponding Architecture/Microarchitecture     | `%_electron_cpu` |
  |:------------------------------:|:----------------------------------------------------:|:----------------:|
