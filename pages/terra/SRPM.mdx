...
- It MUST always come after `Name:` is defined, but MAY need to come before the sources if an upstream provides sources named after the Electron architecture.
- It SHOULD come before the `License:` field if using [`%{electron_license}`](#electron_license).
- It MAY be used under preambles but before `%prep` if not using `%{electron_license}`.
