...
### %git_clone
Downloads the source code via `git clone`, and passes useful flags such as`--depth 1`, `-q`, `--recursive-submodules`, and `-j${RPM_BUILD_NCPUS}`.
A URL and a version or commit hash can be passed to this macro.
