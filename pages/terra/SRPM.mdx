...
## Directories
These macros define certain system directories.
...
The common, albeit clunky, solution is to define `%{builddir}` manually as `%{_builddir}` in specs targeting distros with different versions of RPM where one has a version older than 4.20. We have this macro instead to make the process easier.
...
- `-l` can be used to fetch all licenses including the bundled ones and put them in a directory called `bundled_licenses`. You can then put them in the license directory using `%license bundled_licenses/*`
...
### Node.js Macros
Macros for Node.js itself. These require use of the existing Node.js macros in Fedora's `nodejs-packaging` package.
...
Accepts an `-o` flag as a shorthand for `--out`.
...
Based on Fedora's `%set_build_flags`.
...
Available flags:
- `-c` will run `%{__npm} ci` instead of `%{__npm} install`. May be necessary for projects with stricter build dependencies.
...
#### %bun_common_envvars
Sets the `$BUN_HOME` for when this feature becomes available and disables transpiler cache.
...
You DON'T need to fix vulnerabilities for Node modules that are __not bundled in the final package.__ This means if a vulnerability is found *only* in build dependencies, you are fine to leave them be.

#### %bun_pm_trust
Add Node modules to trusted dependencies so their postinstall scripts may be run. Should only be used if necessary.

Available flags:
- `-a` can be used to trust all dependencies, otherwise simply list the depdencies you want to trust after the command.
...
#### %vendor_pnpm
Use if PNPM is not usable/available from the available repositories.
...
- `-N`: Executes the `%{__yarn} install` step with `--no-lockfile`. Do not update a lockfile on depedency installation.
...
`%git_clone` should only be used when downloading a compressed source cannot be used (for example, if the build process expects a git environment or the release tarball doesn't contain submodules).
