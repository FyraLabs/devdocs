...
Like other meta macros (e.g., `%forgemeta` from Fedora), you MUST be mindful of where in the spec this macro should be placed. It MUST always come after `Name:` is defined, but MAY need to come before the sources if an upstream provides sources named after the Electron architecture. It also SHOULD come before the `License:` field if using [`%{electron_license}`](#electron_license).
