...
SHOULD NOT be used in a spec file, only used as a macro within [`%pkg_devel_files`](#pkg_devel_files).

### %__anda_libsfiles
Expands to: `%_anda_srpm_macros_dir/find-libsinfo.sh %buildroot > anda-libsfiles.list`
SHOULD NOT be used in a spec file, only used as a macro within [`%pkg_libs_files`](#pkg_libs_files).

### %_anda_srpm_macros_dir
Expands to: `/usr/libexec/anda-srpm-macros`
Anda directory for scripts such as `find-develinfo.sh` used by Anda SRPM macros.

### %__anda_staticfiles
Expands to: `%_anda_srpm_macros_dir/find-staticinfo.sh %buildroot > anda-staticfiles.list{:rpmspec}`.
Creates anda-staticfiles.list file in `%{buildroot}{:rpmspec}` to be used by `%pkg_static_files{:rpmspec}`.

### %_appsdir
Expands to: `%_datadir/applications{:rpmspec}` or `/usr/share/applications{:rpmspec}`.
This may seem like a directory to place the application itself into, but this directory is used for applicaiton's `.desktop` files.
...
#### %_nvm_dir
The location in the `%{rpmbuilddir}{:rpmspec}` what will be passed to `$NVM_DIR{:rpmspec}`.
...
You MUST add `nvm` as a build dependancy: `BuildRequires: nvm{:rpmspec}`.
...
#### %npm_test
Runs generic NPM tests. This will not be applicable to all Node modules. For use in `%check{:rpmspec}`.
...
### %evr
Used for declaring how a package interacts with others, for example, in a `Requires:{:rpmspec}` or `Obsoletes:{:rpmspec}` tag.
