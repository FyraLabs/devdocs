...
- Submissions SHOULD be placed in the corresponding directory in [`anda/`](https://github.com/terrapkg/packages/tree/frawhide/anda).
...
You MUST test your package locally.
You can install the built `.rpm` file by running `dnf install path/to/package.rpm`.
An example of why this needs to be done is, a package could build, but then run into runtime dependancy error.
