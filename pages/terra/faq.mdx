...
- Automation that tracks, updates, and backports packages, reducing the time between upstream release and distribution to end-users. Additionally, this reduces a signifigant burden for maintainers, who have lives too ;p.
...
- [RPMFusion](https://rpmfusion.org/) is a great repository, but it doesn't have the level of openness and transparency we're aiming for. Additionally, Terra runs on a rolling release model, which RPMFusion doesn't claim to meet.
...
Replace `rawhide` with your Fedora release version.
