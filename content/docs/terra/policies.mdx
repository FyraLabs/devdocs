...
Only [submit](/terra/policy#submission-policy) packages legally distributable, proprietary or not, but try not to PR things that are (a) already in Fedora, & (b) WIP / untested code.
...
- Submissions must be maintained. We will reject submissions of unmaintained software. The only exception to this SHOULD be required dependancies of modern applications.
...
<Callout type="idea">Our AppStream guidelines are documented [here](/terra/appstream).</Callout>
