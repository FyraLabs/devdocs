...
<Callout title="Macro Argument Syntax" type="important">
...
<Callout title="Important Note for Tauri Macros" type="idea">
  Take notice of the section [System Tauri Macros](#system-tauri-macros)' description. Those are macros for using Tauri *installed via the repository,* AKA installed to the system.<br />
  ...
  TL;DR: Use [System Tauri Macros](#system-tauri-macros) for Cargo Tauri installed from the repository, use [Webapp Build Macros](#webapp-build-macros) for JavaSript Tauri installed as a JS module dependency, and the latter will fit most usecases.
</Callout>
...
<Callout type="important">
  All webapp build macros run the `install` step for dependencies as part of their build process, this is never manually required when using them.
</Callout>
...
It is RECOMMENDED to still include a source tarball even if it is not used so that the source RPM is not empty.
