...
export function SearchTerra({ macroname }) {
  ...
  // Helper function for generating a sentence fpr quicksearching Terra for the macro you pass it
  ...
  return (
  <span>
    Click <a href={url}>here</a> to see all examples of <code className="language-rpm">%{macroname}</code>.
  </span>
  );
}
