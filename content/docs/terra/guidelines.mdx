...
C[%tauri_prep];
subgraph BB [Build Macros];
D[Webapp Build Macro];
E[%tauri_build];
end
subgraph CC [Install Macros];
F[%electron_install];
G[%tauri_install];
end
...
C-- Installing an Electron App -->F;
C-- Installing a Tauri App -->G;
E-->G;
G-->H;"
/>
...
A -- Using JS Module Tauri -->B;
A -- Using System Tauri -->C;
B-->D;
C-->D;
D-->E;"
/>
