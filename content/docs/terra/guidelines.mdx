...
```mermaid
graph TD;
  Electron-->Webapp build macro-->%electron_install
  Tauri JavaScript-->%tauri_prep-->Webapp build macro-->%tauri_install
  Tauri Cargo-->%tauri_prep-->%tauri_build-->%tauri_install
```
