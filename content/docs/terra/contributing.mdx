...
1. Install the Dev Containers extension in your IDE
   ...
   a. Zed comes with Dev Containers, see [this documentation](https://zed.dev/docs/dev-containers)

   b. VSCode users need to install the Dev Containers extension

2. Get the container runtime

   a. On Windows and Mac, install [Docker Desktop](https://docs.docker.com/desktop/)

   b. On Linux, install `podman-docker` from your package manager

3. Fork the [Terra packages repo](https://github.com/terrapkg/packages), and `git clone` your fork
...
<Callout type="info">
  If VSCode doesn't prompt you, press `Ctrl` + `Shift` + `P`, type `Reopen in
  Container`, and press `Enter`
</Callout>
