...
// For GitHub repos with tags but not releases, use `gh_tag()` instead:
...
let latest_ver = gitlab_commit("gitlab.gnome.org", "1551", "main");
let latest_tag = gitlab_tag("25716028");
// WARN: There is no `gitlab_rawfile()`.

// The same functions exist for Codeberg. Simply replace `gh` with
// `codeberg`.
let latest_ver = codeberg("GeopJr/BLAHAJ");
let latest_tag = codeberg_tag("GeopJr/BLAHAJ");
...
let latest_tag = gitea_tag("git.pika-os.com", "general-packages/falcond");
let latest_commit = gitea_commit("git.pika-os.com", "general-packages/falcond");
// WARN: There is no `gitea_rawfile()`.

// Sourcehut is here too! :)
let latest_ver = sourcehut("~kennylevinsen/greetd");
// WARN: There is no `sourcehut_tag()`.
let latest_commit = sourcehut_commit("~kennylevinsen/greetd");
let raw_readme_content = sourcehut_rawfile("~kennylevinsen/greetd", master, "README.md");

// Just like Gitea, Forgejo requires specifiying a custom domain, and has the same functions available.
let latest_ver = forgejo("forgejo.ellis.link", "continuwuation/continuwuity");
let latest_tag = forgejo_tag("forgejo.ellis.link", "continuwuation/continuwuity");
let latest_commit = forgejo_commit("forgejo.ellis.link", "continuwuation/continuwuity");
// WARN: There is no `forgejo_rawfile()`.

// Sourcearcade functions
let sourcearcade_latest = sourcearcade("flashprog");
let sourcearcade_tag_latest = sourcearcade_tag("flashprog");
let sourcearcade_commit_latest = sourcearcade_commit("flashprog");
let sourcearcade_readme = sourcearcade_rawfile("flashprog", "main", "README.md");

// Or you can fetch plain Git tags from any forge
let andaman_tags = git_tags("https://github.com/FyraLabs/anda");
...
let opam_latest = opam("dune");
let cran_latest = cran("jsonlite");
let gnome_extensions_latest_version = gnome_extensions("<uuid>");
let hex_latest = hex("phoenix");
