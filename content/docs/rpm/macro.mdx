...
Escaping characters in regex inside RPM macros has some notable differences from regular regex usage especially when it comes to escapes. These are imporant to be aware of when using exlusions macros such as `%__requires_exclude`.
...
In [Lua macros](/rpm/macros#lua) which are in a macros file (*not* in a spec!) this can get even more complicated as you must sometimes escape in Lua *and* RPM.
