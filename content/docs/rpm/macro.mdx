...
Escaping characters in regex inside RPM macros has some notable differences from regular regex usage, especially when it comes to escapes. These are imporant to be aware of when using exlusion macros such as `%__requires_exclude`.
