...
<Split.Root>
	<div>
		```rpmspec title="/usr/lib/rpm/macros.d/macros.example"
		%say_hello echo \\\
		Hello, World! && \\\
...
```rpmspec title="/usr/lib/rpm/macros.d/macros.rust-srpm" showLineNumbers {14,18,23}
...
| Invocation | `%global x 0{:rpmspec}` | `%undefine x{:rpmspec}` |
| ---------- | ----------------------- | ----------------------- |
| `%{?x:a}`  | `a`                     | `` ``                   |
| `%{!?x:a}` | `` ``                   | `a`					 |
