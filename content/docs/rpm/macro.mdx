...
Regex in RPM has some notable differences from regular regex usage especially when it comes to escapes. These are imporant to be aware of when using exlusions macros such as `%__requires_exclude`.
